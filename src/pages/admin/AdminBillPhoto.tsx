...
export default function AdminBillPhoto() {
  ...
  async function uploadImage(file: File) {
    ...
    if (error) {
      setUploadError("Photo upload failed: " + error.message + " (Supabase में bill-images bucket बनाएं)");
      setUploading(false);
      return;
    }
  ...
  async function saveBill() {
    ...
    if (!quantity || !price) { setSaveError("Enter quantity and price"); return; }
    ...
    if (error) { setSaveError("Not saved: " + error.message); setSaving(false); return; }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-gray-500 font-hindi text-sm">Upload bill photo and fill manual entry. OCR/AI auto-entry can be added later.</p>

      {saved && (
        <div className="bg-green-50 border-2 border-green-400 rounded-2xl p-4 flex items-center gap-3">
          <CheckCircle className="w-6 h-6 text-green-600" />
          <p className="text-green-800 font-hindi font-bold">{t("billCreated")} Bill saved!</p>
        ...
        <div className="space-y-3">
          <div
            ...
            {uploading ? (
              ...
              <div className="space-y-3">
                ...
                <p className="font-hindi text-gray-400 text-sm">Drop a photo here or click</p>
                <p className="font-hindi text-gray-300 text-xs">JPG, PNG, WEBP supported</p>
              </div>
            )}
          </div>
          ...
          {uploadError && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-3">
              ...
              <p className="text-red-400 text-xs font-hindi mt-1">Note: entry can still be saved without photo upload.</p>
            </div>
          )}
          ...
          {!imagePreview && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="font-hindi font-bold text-yellow-800">OCR / AI Auto-entry</p>
              </div>
              <p className="font-hindi text-yellow-700 text-sm">In future, bill photo data can be filled automatically. For now, fill manual entry.</p>
            </div>
          )}
        </div>
